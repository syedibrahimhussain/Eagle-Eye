...
try:
	...
	while capture.isOpened():
	    ...
	    if ret == True:
	        cv2.imshow('Processing Video from dpv',frame)
	        cv2.resizeWindow('Processing Video from dpv', 1024, 720)
	        cv2.imwrite("./output/frame%d.jpg" % count, frame)
	        count += 1
	        if cv2.waitKey(10) and 0xFF == ord('q') or count == 150:
	            break
	    else:
	        break
	capture.release()
...
fig.suptitle('Processed Images from dpv')
